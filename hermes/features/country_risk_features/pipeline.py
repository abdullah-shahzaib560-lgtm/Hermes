    ...
    def get_country_risk_features(self, country):
        ...
        return {
            ...
            "geopolitical": {
                ...
                "conflict_trend": self.geo.conflict_trend(country_code=country, mode='F'),                                        # string: escalating/stable/de-escalating
                "goldstein_scale_avg_30d": self.geo.goldstein_scale_avg_30d(country_code=country, mode='F'),                      # float, -10 to +10
                "goldstein_scale_trend": self.geo.goldstein_scale_trend(country_code=country, mode='F'),                          # float, change vs prev period
                ...
                "diplomatic_event_count_30d": self.geo.diplomatic_event_count_30d(country_code=country, mode='F'),                # int
                "diplomatic_intensity_avg": self.geo.diplomatic_intensity_avg(country_code=country, mode='F'),                    # float, 0-10 scale
                ...
                "sanctions_sector_coverage": self.geo.sanctions_sector_coverage(country_code=country, mode='F'),                  # float, 0-1
                "governance_wgi_composite": self.geo.governance_wgi_composite(country_code=country, mode='F'),                    # float, -2.5 to +2.5
                "corruption_perception_index": self.geo.corruption_perception_index(country_code=country, mode='F'),              # int, 0-100
                "rule_of_law_score": self.geo.rule_of_law_score(country_code=country, mode='F'),                                  # float, -2.5 to +2.5
                "regulatory_quality": self.geo.regulatory_quality(country_code=country, mode='F'),                                # float, -2.5 to +2.5
                "democracy_index": self.geo.democracy_index(country_code=country, mode='F'),                                      # float, 0-1
                "regime_type": self.geo.regime_type(country_code=country, mode='F'),                                              # string: democracy/hybrid/autocracy
                "press_freedom_score": self.geo.press_freedom_score(country_code=country, mode='F'),                              # int, 0-100
            },
            "security": {
                "military_spending_gdp": self.sec.military_spending_gdp(country_code=country, mode='F'),                          # float, percent
                "military_spending_growth_yoy": self.sec.military_spending_growth_yoy(country_code=country, mode='F'),            # float, percent
                "alliance_strength_score": self.sec.alliance_strength_score(country_code=country, mode='F'),                      # float, 0-1
            ...
            "social": {
                ...
                "human_development_index": self.soc.human_development_index(country_code=country, mode='F'),                      # float, 0-1
                "gini_coefficient": self.soc.gini_coefficient(country_code=country, mode='F'),                                    # float, 0-100
                "poverty_headcount_ratio": self.soc.poverty_headcount_ratio(country_code=country, mode='F'),                      # float, percent
            },
            "environmental": {
                "climate_vulnerability_score": self.env.climate_vulnerability_score(country_code=country, mode='F'),              # float, 0-1
                "climate_readiness_score": self.env.climate_readiness_score(country_code=country, mode='F'),                      # float, 0-1
                "natural_disaster_risk": self.env.natural_disaster_risk(country_code=country, mode='F'),                          # float, 0-1
                "food_price_index_change_yoy": self.env.food_price_index_change_yoy(country_code=country, mode='F'),              # float, percent
                "energy_dependence_ratio": self.env.energy_dependence_ratio(country_code=country, mode='F'),                      # float, 0-1 (imports/consumption)
